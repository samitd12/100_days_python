...
def calculate_score(num):
    ...
    if sum(num) ==21 and len(num)==2:
        return 0
    if 11 in num and sum(num) >21:
        cards.remove(11)
        cards.append(1)
    return sum(num)
...
#Hint 5: Deal the user and computer 2 cards each using deal_card() and append().
    ...
    while not is_game_over:
        ...
        if user_cards_sum == 0 or computer_cards_sum == 0 or user_cards_sum>21:
            ...
            user_deal=input("Tyoe 'y' to get another card, type 'n' to pass" )
            if user_deal == "y":
                user_cards.append(random_card())
            else:
                is_game_over = True
    ...
    print(compare(user_cards_sum,computer_cards_sum))
